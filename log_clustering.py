...
class LogTemplateExtractor:
    ...
    def setDelimiter(self, delimiter):
        '''
        Set the delimiters (in regular expression) for dividing one log into tokens.
        '''
        self.delimiter = delimiter
        
    def setDistanceThreshold(self, distance_threshold):
        '''
        Set the distance threshold used for creating new cluster. The value range is 0 ~ 1. 
        The less the threshold is, the more similar two logs have to be if they want to be clustered together.
        '''
    ...
    def minDistance(self, added_line, cluster_dict):
        ...
        '''
        distance = []
        added_line_tokens = self.replaceNumByWildcard(re.split(self.delimiter, added_line[self.ignored_chars:]))
        for cluster_num in cluster_dict:
            cluster = cluster_dict[cluster_num]
            cluster_line_tokens = self.replaceNumByWildcard(re.split(self.delimiter, cluster[0][self.ignored_chars:]))
            ...
            for i in command_cluster: 
                for line in command_cluster[i]:
                    if not cluster_dict:
                        ...
                        min_dis, min_index = self.minDistance(line, cluster_dict)
